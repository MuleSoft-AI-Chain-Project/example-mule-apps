...
const ToxicityDisplay = ({ toxicity }) => {
...
const RenderMessage = ({ message }) => {
  ...
  return (
    <div className={`flex flex-col mb-2 ${alignClass} max-w-3xl lg:max-w-4xl animate-fadeIn`}>
      <div className={`${bubbleClass} p-6 rounded-3xl border border-gray-700/50 shadow-md`}>
        <div className="text-white text-base text-left leading-relaxed">
          {typeof message.content === "string" ? (
            ...
            <>
              ...
              >
                {message.partialResponse || message.content.payload.response || ""}
              </ReactMarkdown>
              ...
              {message.content.payload.sources &&
                message.content.payload.sources.length > 0 && (
                  <div className="mt-4">
                    <h3 className="text-xs text-gray-300 font-medium mb-2">Sources:</h3>
...
const transformSources = (sources) => {
  if (!sources || !Array.isArray(sources)) return [];
  
  return sources.map((source, index) => ({
    fileName: source.metadata?.source_id || `Source ${index + 1}`,
    textSegment: source.text || '',
    score: source.score,
    metadata: source.metadata
  }));
};

export default function QueryStore({ className = "", storeNames, llmSettings }) {
  ...
  const handleRecordingComplete = useCallback(
    async (response) => {
      ...
      // Transform sources before passing to render
      const transformedResponse = {
        ...response,
        payload: {
          ...response.payload,
          sources: transformSources(response.payload.sources)
        }
      };

      simulateStreamingResponse(response.payload.response, agentIndex, transformedResponse);
    },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const transformedData = {
        ...
        payload: {
          ...data.payload,
          sources: transformSources(data.payload.sources)
        }
      };

      simulateStreamingResponse(data.payload.response, agentIndex, transformedData);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
  ...
  const simulateStreamingResponse = (response, messageIndex, originalMessage) => {
    let currentText = "";
  ...
  return (
    ...
    >
      ...
      <div className="flex-none p-6 pt-2 border-t border-gray-800/40" data-form-type="other">
        <div className="flex items-center mb-4">
          <label htmlFor="store-select" className="text-white text-sm mr-3 flex-none">
            Knowledge Store
          </label>
          <select
            id="store-select"
